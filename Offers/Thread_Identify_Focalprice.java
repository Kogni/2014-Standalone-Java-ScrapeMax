...
public class Thread_Identify_Focalprice extends Thread_Identify{

	public Thread_Identify_Focalprice( Controller Class_Controller, Object_SearchJob hentetJobb ) {
		...
		while ( EditedBuffer.indexOf( ".jpg" ) > -1 ) {
			...
			if ( Bilde.lastIndexOf( "http" ) > -1 ) {
				...
				if ( Bilde.length() > 100 ) {
					...
					//Jobb.TargetOffer.ImageText = Jobb.TargetOffer.ImageText + "<img src=\""+Bilde+"\">";
	...
	public void FinnPris( String Buffer ) throws Exception {
		...
		if ( Buffer.indexOf( "nowprice\">".toLowerCase() ) > -1 ) {
			...
			String TotalCosts_StringA = Buffer.substring( (Buffer.indexOf( "nowprice\">" ) + "nowprice\">".length()) );
			//8.92</span>
			TotalCosts_StringA = TotalCosts_StringA.substring( 0, TotalCosts_StringA.indexOf( "</span>" ) );
			String TotalCosts_StringB = TotalCosts_StringA;
			if ( TotalCosts_StringB.indexOf( "<sup>" ) > -1 ) {
				TotalCosts_StringB = TotalCosts_StringB.replaceAll( "<sup>", "" );
			}
			if ( TotalCosts_StringB.indexOf( "</sup>" ) > -1 ) {
				TotalCosts_StringB = TotalCosts_StringB.replaceAll( "</sup>", "" );
			}
			String TotalCosts_StringC = TotalCosts_StringB;
			//6..69
			if ( TotalCosts_StringC.indexOf( ".." ) > -1 ) {
				TotalCosts_StringC = TotalCosts_StringC.replaceAll( "..", "." );
			}
			try {
				double TotalCosts_doub = Double.parseDouble( TotalCosts_StringC );
				...
				//System.out.println( this.getClass().toString() + " FinnPris klarte ikke finne pris: C=" + TotalCosts_StringC + " B=" + TotalCosts_StringB + " A=" + TotalCosts_StringA + " " + this.Jobb.TargetOffer.Get_Adresse().Get_Adresse().toString() );
