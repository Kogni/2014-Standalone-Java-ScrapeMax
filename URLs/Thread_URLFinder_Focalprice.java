...
public class Thread_URLFinder_Focalprice extends Thread_URLFinder {
	
	public Thread_URLFinder_Focalprice(Controller class_Controller, Object_SearchJob hentetJobb) {
	...
	public void SplitLine1( String Pagecontent ) throws Exception {
		...
		while ( token.hasMoreTokens() ){
			...
			while ( m.find() ) {
				...
				if ( ProperUrl( "http://www.focalprice.com"+AddresseURL ) == true ) {
					AddresseURL = "http://www.focalprice.com"+AddresseURL;
					count++;
					Jobb.LinksOut ++;
				...
				} else if ( ProperUrl( "http://www.focalprice.com/"+AddresseURL ) == true ) {
					count++;
					Jobb.LinksOut ++;
					Object_URL nyURL = new Object_URL( AddresseURL, FigureRelatedValue( AddresseURL ), Class_Controller, Jobb.TargetOffer.Get_Adresse(), Jobb.TargetOffer.Get_Adresse().Layer+1 );
					this.Class_Controller.SaveNewURL( nyURL );
